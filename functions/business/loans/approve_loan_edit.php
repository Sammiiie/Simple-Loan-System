...
if (isset($_GET['approve'])) {
    ...
    $updateLoan = update('loan', $loanId, 'idloan', $loanEdit);
    if (!$updateLoan) {
        $error = "Error: \n" . mysqli_error($connection); //checking for errors
            $_SESSION["feedback"] = "Something went wrong could not edit loan! - $error";
        ...
        if (!$approveEdit) {
            $error = "Error: \n" . mysqli_error($connection); //checking for errors
            $_SESSION["feedback"] = "Sorry could not Approve loan edit! - $error";
            $_SESSION["Lack_of_intfund_$randms"] = "10";
            echo header("Location: ../../../edit_approval.php?message1=$randms");
            exit();
        } else {
            $_SESSION["feedback"] = "Loan Successfuly Edited";
            $_SESSION["Lack_of_intfund_$randms"] = "10";
